...
#else
	*Data =  read_flash((VirtualAddress*4)+_EEPROM_START_FLASH_PAGE_ADDRESS);
#endif
	return true;
}
//##########################################################################################################
...
#else
		*Data =  read_flash((i*4)+_EEPROM_START_FLASH_PAGE_ADDRESS);
#endif
		Data++;
	}
	return true;
...
#else
		temp.fbyte =  read_flash((i*4)+_EEPROM_START_FLASH_PAGE_ADDRESS);
#endif
		// Fill bytes
...
#else
  if(read_flash((VirtualAddress*4)+_EEPROM_START_FLASH_PAGE_ADDRESS) != 0xFFFFFFFF)
#endif
	{
...
#else
		  if(read_flash((i*4)+_EEPROM_START_FLASH_PAGE_ADDRESS) != 0xFFFFFFFF)
#endif
		{
			//create backup of that page excluding bad data
...
#else
		tempdata =  read_flash((i*4)+PageAddress);
#endif
...
#else
		tempdata =  read_flash((i*4)+_EEPROM_TRANSFER_PAGE_ADDRESS);
#endif
